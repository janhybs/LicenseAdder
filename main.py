...
class LicenseManager:
    ...
    def normalizePaths (self, pathPrefix, locations) :
        normalized = [self.normalizePath (pathPrefix, location) for location in locations]
        return normalized


    def normalizePath (self, pathPrefix, location) :
        if not location : return None
        return os.path.abspath (os.path.join (pathPrefix, location))


    def checkExistance (self, locations, skipError=True) :
        errors = []
    ...
    def checkExtension (self, file, exts) :
        file = file.upper ()
        for ext in exts :
    ...
    def handleFile (self, file, opts) :
        print 'doing ' + file
        content = self.loadTrimmed (file)
    ...
    def loadLicense (self, oldlicense) :
        if not oldlicense :
            return None
        return open (oldlicense, "r").read ()


    def handleFiles (self, files, opts, root="", level=1) :
        exts = opts['exts']
        for file in files :
            ...
            if os.path.isdir (realfile) :
                self.handleFiles (os.listdir (realfile), opts, realfile, level + 1)
            else :
                # no rules or correct extension
                if not exts or self.checkExtension (realfile, exts) :
                    self.handleFile (realfile, opts)
                else :
                    # print ("file skipped " + file)
                    pass


    def loadTrimmed (self, file) :
        if not file :
            return None
    ...
    def run (self):
        ...
        print (options)
        files = self.normalizePaths (options.path, options.files)
        dirs = self.normalizePaths (options.path, options.dirs)
        oldlicense = self.loadLicense (self.normalizePath (options.path, options.oldlicense))
        newlicense = self.loadLicense (self.normalizePath (options.path, options.newlicense))
        ...
        self.checkExistance (files + dirs, options.skip)
        opts = { 'exts' : exts, 'oldlicense' : oldlicense, 'newlicense' : newlicense }

        print 'solo files: '
        self.handleFiles (files, opts)

        print 'dirs: '
        self.handleFiles (dirs, opts)
